...
# Traffic-profiler policy. These defaults match the live 2-second aggregation
# window and can be tuned per deployment without retraining NetDreamer.
TRAFFIC_VOLUME_THRESHOLD = int(os.getenv("TRAFFIC_VOLUME_THRESHOLD", "300"))
HIGH_SOURCE_ENTROPY = float(os.getenv("HIGH_SOURCE_ENTROPY", "4.0"))
LOW_SOURCE_ENTROPY = float(os.getenv("LOW_SOURCE_ENTROPY", "2.2"))
BALANCED_SYN_ACK_RATIO = float(os.getenv("BALANCED_SYN_ACK_RATIO", "1.5"))
...
feature_sequences = defaultdict(lambda: deque(maxlen=SEQUENCE_LENGTH))
...
def calculate_shannon_entropy(items):
    """Return base-2 Shannon entropy for a collection of categorical values."""
...
def profile_window_traffic(packets):
    """Calculate diversity, handshake, byte-direction, and timing metrics."""
    source_ips = []
    destination_ports = []
    timestamps = []
    syn_count = 0
    ...
    ingress_bytes = 0
    egress_bytes = 0
    protected_ips = {DEFENSE_IP, GATEWAY_IP, INTERNAL_SERVER_IP, "127.0.0.1", "::1"}

    for packet in packets:
        src = packet.get("src")
        dst = packet.get("dst")
        dst_port = packet.get("dst_port")
        flags = int(packet.get("flags", 0) or 0)
        payload_length = max(0, int(packet.get("payload_length", 0) or 0))

        if src:
            source_ips.append(src)
        if dst_port is not None:
            destination_ports.append(dst_port)
        if packet.get("time") is not None:
            timestamps.append(float(packet["time"]))
        if flags & 0x02:
            syn_count += 1
        if flags & 0x10:
            ack_count += 1
        if dst in protected_ips:
            ingress_bytes += payload_length
        if src in protected_ips:
            egress_bytes += payload_length

    # A denominator floor keeps telemetry valid JSON while still making a
    # SYN-only or ingress-only stream strongly asymmetric.
    syn_ack_ratio = float(syn_count) / float(max(ack_count, 1))
    ingress_egress_byte_ratio = float(ingress_bytes) / float(max(egress_bytes, 1))

    iat_jitter = 0.0
    if len(timestamps) > 2:
        inter_arrivals = np.diff(np.sort(np.asarray(timestamps, dtype=np.float64)))
        mean_iat = float(np.mean(inter_arrivals))
        if mean_iat > 0.0:
            iat_jitter = float(np.std(inter_arrivals) / mean_iat)

    return {
        "total_packets": len(packets),
        "unique_source_ips": len(set(source_ips)),
        "unique_destination_ports": len(set(destination_ports)),
        "source_ip_entropy": calculate_shannon_entropy(source_ips),
        "destination_port_entropy": calculate_shannon_entropy(destination_ports),
        "syn_count": syn_count,
        "ack_count": ack_count,
        "syn_ack_ratio": syn_ack_ratio,
        "ingress_payload_bytes": ingress_bytes,
        "egress_payload_bytes": egress_bytes,
        "ingress_egress_payload_ratio": ingress_egress_byte_ratio,
        "inter_arrival_jitter": iat_jitter,
    }


def classify_traffic_profile(profile):
    """Apply the deterministic high-volume guardrail to profiler metrics."""
    if profile["total_packets"] <= TRAFFIC_VOLUME_THRESHOLD:
        return "NORMAL_TRAFFIC"

    if (
        profile["source_ip_entropy"] > HIGH_SOURCE_ENTROPY
        and profile["syn_ack_ratio"] <= BALANCED_SYN_ACK_RATIO
    ):
        return "LEGITIMATE_FLASH_CROWD"

    if (
        profile["source_ip_entropy"] < LOW_SOURCE_ENTROPY
        or profile["syn_ack_ratio"] > ASYMMETRIC_SYN_ACK_RATIO
    ):
...
def save_state(src_ip, label_name, pred_proba, future_threat_score, is_isolated,
    ...
    state_data = {
        ...
        "raw_model_risk": round(float(raw_model_risk if raw_model_risk is not None else future_threat_score), 4),
    ...
    try:
        ...
        if proto == 6:  # TCP
            ...
            if len(data) >= tcp_offset + 20:
                ...
                flags = tcp_hdr[13]
                tcp_header_length = (tcp_hdr[12] >> 4) * 4
                payload_length = max(0, total_len - ihl - tcp_header_length)
        elif proto == 17:  # UDP
            ...
            try:
                ...
                if hasattr(packet, 'tcp'):
                    ...
                    if hasattr(packet.udp, 'length'):
                        payload_length = max(0, int(packet.udp.length) - 8)
                ...
                if src_ip:
                    ...
                    if feature_array is not None:
                        ...
                        try:
                            ...
                            source_sequence = feature_sequences[src_ip]
                            source_sequence.append(scaled_feats[0])
                            sequence_rows = list(source_sequence)
                            if len(sequence_rows) < SEQUENCE_LENGTH:
                                sequence_rows = [sequence_rows[0]] * (SEQUENCE_LENGTH - len(sequence_rows)) + sequence_rows
                            sequence_array = np.asarray(sequence_rows[-SEQUENCE_LENGTH:], dtype=np.float32)
                            x_tensor = torch.tensor(sequence_array, dtype=torch.float32).unsqueeze(0)
                            with torch.no_grad():
                    ...
                    # 3. Post-inference traffic-context guardrail. Preserve the
                    # raw neural risk for auditability while adjusting only the
                    # operational risk used by telemetry and SOAR.
                    raw_model_risk = nn_risk
                    guardrail_action = "NONE"
                    if traffic_profile == "LEGITIMATE_FLASH_CROWD":
                        label_name = "Legitimate Flash Crowd / High Concurrency"
                    ...
                    save_state(
                        ...
                        dict(ip_counts), rollout_values=rollout_list,
                        raw_model_risk=raw_model_risk,
                    ...
                    is_threat = (label_name not in ("Benign", "Legitimate Flash Crowd / High Concurrency"))
                    profile_changed = traffic_profile != last_traffic_profile
                    if is_threat or (now - last_log_time >= 3.0) or (label_name != last_threat_state) or profile_changed:
                        if traffic_profile == "LEGITIMATE_FLASH_CROWD":
                            write_log("[TRAFFIC PROFILER] High packet velocity detected with high IP entropy. Classified as organic flash crowd. Suppressing false alarm.")
                        elif traffic_profile == "VOLUMETRIC_ATTACK":
                            write_log(f"[TRAFFIC PROFILER] Low source diversity or asymmetric TCP handshakes detected. Classified as volumetric attack / coordinated flood. Entropy: {traffic_metrics['source_ip_entropy']:.2f} | SYN/ACK: {traffic_metrics['syn_ack_ratio']:.2f}")
                        if is_threat:
                        ...
                        last_traffic_profile = traffic_profile
